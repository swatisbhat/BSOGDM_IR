...
class BSO:
    ...
    def bso(self, doc_clusters, doc_term_indices, freq_patterns,query,terms_map, solution_size, max_iter):
        ...
        BeeInit.sort(key = lambda x: doc_probabilities[x],reverse=True)
        soln = BeeInit[:solution_size]
        prev_fitness = fitness_val
        fitness_val = self.calc_fitness(soln, doc_term_indices, query_term_indices)
