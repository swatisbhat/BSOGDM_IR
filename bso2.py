...
class BSO:
    ...
    def bso(self, doc_clusters, doc_term_indices, freq_patterns,query,terms_map, solution_size, max_iter):
        ...
        for i in range(no_of_clusters):
            for j in doc_clusters[i]:
                ...
                if len(relevant_terms[i])>0:
                    doc_probabilities.append(float(prob)/len(relevant_terms[i]))
                else:
                    doc_probabilities.append(0)
        ...
        fitness_val = 0
        prev_fitness = 0
        while number_of_iterations < Max and abs(fitness_val - prev_fitness) < 0.008:
            check_sources = 0
            for i in sources:
                if len(i)!=0:
                    check_sources = 1
